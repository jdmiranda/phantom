...
impl App {
    ...
    /// Split the focused terminal pane into two when it enters alt-screen mode.
    ///
    /// The primary pane (source) keeps its normal-screen buffer; a new sibling
    /// `AltScreenViewAdapter` pane mirrors the alt-screen program. Both share
    /// a `Arc<Mutex<Option<RenderOutput>>>` snapshot written by the primary on
    /// each `update()` tick.
    ///
    /// Returns the `AppId` of the newly-created view pane, or `None` on failure.
