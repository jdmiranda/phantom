...
//!
//! Used by issue #2 (process detach v1): when a terminal enters alt-screen
//! mode (vim, htop, less), the primary pane renders the normal-screen history
//! while a sibling `AltScreenViewAdapter` renders the alt-screen program.
//! Both share the same `Arc<Mutex<Option<RenderOutput>>>` snapshot that the
//! primary writes each frame via its `update()` override.
//!
...
mod tests {
    ...
    #[test]
    fn get_state_contains_type_and_label() {
        let snap = new_snapshot();
        let adapter = AltScreenViewAdapter::new(snap, "htop".into());
        let state = adapter.get_state();
        assert_eq!(state["type"], "alt_screen_view");
        assert_eq!(state["label"], "htop");
    }

    #[test]
    fn title_returns_label() {
        let snap = new_snapshot();
        let adapter = AltScreenViewAdapter::new(snap, "vim".into());
        assert_eq!(adapter.title(), "vim");
    }
    ...
    #[test]
    fn set_app_id_stored() {
        let snap = new_snapshot();
        let mut adapter = AltScreenViewAdapter::new(snap, "vim".into());
        adapter.set_app_id(42);
        // Verify via get_state (no direct accessor, but the adapter should not panic).
        let _ = adapter.get_state();
    }

    #[test]
    fn new_snapshot_starts_empty() {
        let snap = new_snapshot();
    ...
    #[test]
    fn spatial_preference_is_some() {
        let snap = new_snapshot();
        let adapter = AltScreenViewAdapter::new(snap, "vim".into());
        assert!(adapter.spatial_preference().is_some());
    }
}
